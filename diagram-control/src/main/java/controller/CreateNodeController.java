...
/**
 * Used by MainController for handling when a user creates a node using the
 * class or package tool.
 */
...
public class CreateNodeController {
  ...
  public void onTouchPressed(TouchEvent event) {
    ...
    if (event.getSource() instanceof AbstractNodeView) {
      ...
      dragStarts.put(event.getTouchPoint().getId(), new Point2D(event.getTouchPoint().getX() + ((AbstractNodeView) event.getSource()).getX(), event.getTouchPoint().getY() + ((AbstractNodeView) event.getSource()).getY()));
  ...
  public void onTouchReleasedClass(TouchEvent event) {
    Rectangle dragRectangle = dragRectangles.get(event.getTouchPoint().getId());
    diagramController.createNodeView(new ClassNode(dragRectangle.getX(), dragRectangle.getY(), dragRectangle.getWidth(), dragRectangle.getHeight()), false);
  ...
  public void onTouchReleasedPackage(TouchEvent event) {
    Rectangle dragRectangle = dragRectangles.get(event.getTouchPoint().getId());
    diagramController.createNodeView(new PackageNode(dragRectangle.getX(), dragRectangle.getY(), dragRectangle.getWidth(), dragRectangle.getHeight()), false);
    finish(dragRectangle);
  ...
  public void onTouchReleasedLifeline(TouchEvent event) {
    Rectangle dragRectangle = dragRectangles.get(event.getTouchPoint().getId());
    diagramController.createNodeView(new SequenceObject(dragRectangle.getX(), dragRectangle.getY(), dragRectangle.getWidth(), dragRectangle.getHeight()), false);
    finish(dragRectangle);
  }
  ...
  public void onMouseReleasedClass() {
    diagramController.createNodeView(new ClassNode(mouseDragRectangle.getX(), mouseDragRectangle.getY(), mouseDragRectangle.getWidth(), mouseDragRectangle.getHeight()), false);
    finish(mouseDragRectangle);
  }
  
  public void onMouseReleasedPackage() {
    diagramController.createNodeView(new PackageNode(mouseDragRectangle.getX(), mouseDragRectangle.getY(), mouseDragRectangle.getWidth(), mouseDragRectangle.getHeight()), false);
    finish(mouseDragRectangle);
  }
  
  public void onMouseReleasedLifeline() {
    diagramController.createNodeView(new SequenceObject(mouseDragRectangle.getX(), mouseDragRectangle.getY(), mouseDragRectangle.getWidth(), mouseDragRectangle.getHeight()), false);
    finish(mouseDragRectangle);
  }
  ...
  /**
   * Puts the given class node as a child of a package if it is contained in
   * one.
   */
  // TODO This is maybe not necessary, as the graph now adds the child to the
  // package.
