...
/**
 * Used by MainController for handling the recognition of drawn shapes and
 * transforming them in to UML-notations.
 */
public class RecognizeController {
  ...
  public RecognizeController(Pane pDrawPane, AbstractDiagramController pController) {
    ...
    // This is to load the recognizer when starting app, not when starting to
    // draw.
    ...
    for (Sketch s : sketches) {
      if (s.getStroke() != null && s.getStroke().getPoints() != null && !s.getStroke().getPoints().isEmpty()) {
        ...
        if (bestMatchString.equals("Square") || bestMatchString.equals("Rectangle")) {
          ...
          graph.addNode(node, false); // Really don't want to do this until
                                      // mainController.createNodeView but need
                                      // to because of graph.findNode.
        }
        
      }
    }
    
    // Go through all sketches to find edges. Edges need to be recognized after
    // nodes
    for (Sketch s : sketches) {
      if (s.getStroke() != null) {
        recognizer.setStroke(s.getStroke());
        ...
        if (bestMatchString.equals("Line") || bestMatchString.startsWith("Polyline") || bestMatchString.equals("Arc") || bestMatchString.equals("Curve") || bestMatchString.equals("Arrow")) {
