...
public abstract class AbstractDiagramController {
  ...
  // Set to true for allowing classes & packages & sketches to be created with
  // the mouse.
  // With this set to false, multiple users can create these elements while
  // interacting with a touch screen at the same time
   ...
   *
   * @param nodeView
   * @param pCommand Compound command from deleting all selected, if null we
   ...
   *
   * @param edgeView
   * @param pCommand CompoundCommand from deleting all selected, if null we
   *          create our own command.
  ...
  public void deleteEdgeView(AbstractEdgeView edgeView, CompoundCommand pCommand, boolean undo, boolean remote) {
    ...
    if (pCommand == null && !undo) { // If this is not part of a compoundcommand
      command = new CompoundCommand();
      selectedEdges.remove(edgeView); // We can safely delete from selectedEdges
                                      // since we are not looping through it.
    }
    ...
    if (pCommand == null && !undo) { // If this is not part of a compoundcommand
                                     // we add this directly to the UndoManager
      undoManager.add(command);
    }
  }
  ...
  /**
   * Creates a new picture node from the given image and position and adds it to
   * the graph.
   * 
   * @param view
   * @param image
   ...
   * 
   * @param dataArray [0] = Type of change [1] = id of element [2+] = Optional
   *          new values
   */
   ...
   * @param edge
   * @param remote, true if change comes from a remote server
   * @return null if graph already hasEdge or start/endnodeview is null.
   *         Otherwise the created AbstractEdgeView.
   */
